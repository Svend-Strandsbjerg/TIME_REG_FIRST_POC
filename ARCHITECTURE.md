...
- placement state (`uncommitted`, `committed`)
...
- queue projection rules derived from board transitions
...
- Derived outputs: daily totals, queue panel read-model, planned/unplanned summary
...
The app now contains a lightweight queue state to simulate first async behavior without implementing dispatch:

- exactly one queue exists in `BoardState`
- queue status is fixed to `paused`
- queue items are computed deterministically from placement + committed baseline comparison

This keeps queue behavior explicit and testable while preserving future portability.

## 3) Alignment to ASYNC_INTEGRATION_FOUNDATION

Current flow:

`BoardState -> queue simulation projection + TimeEntryDraft[]`

Future flow:

`queue simulation items / drafts -> adapter mapping -> ASYNC_INTEGRATION_FOUNDATION queue contracts`

The current queue item shape intentionally carries minimum integration-relevant fields (queueId, blockId, day, slot, operation) so adapter handoff is straightforward.
